...
from .notifiers import fanout_alert

load_dotenv()

# ── Config ───────────────────────────────────────────────────────────────────
# NOTE: Telegram / Discord config moved out of core. Core fans out via
# HMAC-signed webhooks to NOTIFIER_URLS; the private solguard-notifiers
# service owns channel-specific secrets. See agent/notifiers.py.
...
async def process_event(event: dict):
    ...
    # Fan out to any configured external notifier service (HMAC-signed).
    # Failures here are swallowed — core must never die because a notifier is down.
    try:
        await fanout_alert(record.__dict__)
    except Exception as e:
        print(f"  Notifier fanout failed: {e}")
...
# NOTE: Discord / Telegram fanout moved to the private solguard-notifiers repo.
# Core just HMAC-signs the alert and POSTs to NOTIFIER_URLS. See agent/notifiers.py.
